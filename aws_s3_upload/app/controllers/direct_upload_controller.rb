class DirectUploadController < ApplicationController
  ...
  def create_blob(blob_args)
    ...
    # Dang muon doi key va thu muc luu tru nen them 2 line nay
    pdf_id = SecureRandom.uuid
    blob.update_attribute(:key, "uploads/#{pdf_id}")
    blob
  end

  def signed_url(blob)
    expiration_time = 3.minutes #Pre-singed URL se het han sau 3 phut
    response_signature(
